...
function isStructureValid({ unsafeYear, unsafeMonth, unsafeDay }) {
    ...
    if(yearValid) {
        ...
        if(dayValid && monthValid) {
            // Extra check to verify before 2099-12-27,
            // as it is the last week we can cover entirely
            return unsafeYear !== 2099 || unsafeMonth !== 12 && unsafeDay <= 27;
        }
        return false;
    }
    return false;
...
const MONTH_FRENCH_NAME = ["Janvier", "Février", "Mars", "Avril", "Mai", "Juin", "Juillet", "Août", "Septembre", "Octobre", "Novembre", "Décembre"]
export function monthToFrenchName(month)
{
    const normalized = (Math.max(1, month) - 1) % 12;
    return MONTH_FRENCH_NAME[normalized];
}
